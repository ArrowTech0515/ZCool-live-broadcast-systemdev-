...
const fApi = ref({})
const onlineUsers = ref(0); // Example number of users online
const emit = defineEmits(['emit_clear'])

// Bound variables for form inputs
const lineConfig = ref('');
const lineAddress = ref('');
const userRatio = ref('');
const pageTitle = ref('');
...
<template>
  <a-card style="width: 100%; padding: 0; margin: 1%">
    <a-row type="flex" justify="space-between" align="middle" 
      ...
      <a-col>
        <span style="font-size: 16px; font-weight: bold;">代理客服</span>
      </a-col>
      <a-col>
    ...
    <a-form ref="fApi" :model="fApi" style="padding: 10px;">
      ...
      <a-form-item label="线路地址" :label-col="{ span: 5 }" >
        <a-textarea v-model="lineAddress" placeholder="超链接输入框，必须以 'http' 或 'https' 开头" />
      </a-form-item>
